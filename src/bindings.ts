...
async recordHighlight(request: CreateHighlightRequest) : Promise<Highlight[]> {
return await TAURI_INVOKE("plugin:tauri-specta|record_highlight", { request });
},
async getHighlightsByDate(date: string) : Promise<Highlight[]> {
return await TAURI_INVOKE("plugin:tauri-specta|get_highlights_by_date", { date });
},
async listHighlights() : Promise<Highlight[]> {
return await TAURI_INVOKE("plugin:tauri-specta|list_highlights");
},
async deleteHighlight(ids: number[]) : Promise<null> {
return await TAURI_INVOKE("plugin:tauri-specta|delete_highlight", { ids });
},
...
export type EditHighlightRequest = { id: number; content: string }
